	...
	For Each File In Uploader.Files.Items		
		...
		if lngFileSize = 0 then
			...
			if File.FileSize > lngFileSize then
				bolUpload = false
				strMessage = "Archivo demasiado grande!<br> Redúcelo hasta 250 Kbytes, por favor."
			else
				bolUpload = true
			end if
		end if
		...
		if bolUpload = true then				
		    ...
		    If strExcludes <> "" Then
				If ValidFileExtension(File.FileName, strExcludes) Then
		            strMessage = "No está permitido subir un archivo con la extensión ." & GetFileExtension(File.FileName) & ""
					bolUpload = false
				End If
			End If
